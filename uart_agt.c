...
static irqreturn_t ulite_isr(int irq, void *dev_id)
{
...

	{
		...
		unsigned int TEMP = LSR & 0x00000040;
		unsigned int THRE = LSR & 0x00000020;
		unsigned int DR   = LSR & 0x00000001;
...
{
	unsigned int LSR, TEMP;
	//printk(PREFIX "!!!!!!!! ulite_tx_empty\n");
	LSR = readl(port->membase + AXI_LSR);
	TEMP = LSR & 0x00000040;

	return (TEMP > 0) ? TIOCSER_TEMT : 0;
}
